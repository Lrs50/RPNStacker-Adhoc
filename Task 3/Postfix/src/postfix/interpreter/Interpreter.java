...
public class Interpreter implements Expr.Visitor<Integer> {
	...
	public Integer visitNumberExpr(Expr.Number expr) {
		//Tenta pegar o inteiro da expressão, convertendo diretamente ou pegando do hashmap se for uma variavel com algum valor
		// Se ocorrer algum erro ele retorna exit(-1)
		
		if(Regex.isNum(expr.value)){
			return Integer.parseInt(expr.value);
		}else if(Regex.isVariable((expr.value))){
